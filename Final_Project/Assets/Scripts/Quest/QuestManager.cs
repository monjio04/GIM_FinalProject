...
public class QuestManager : MonoBehaviour
{
    ...
    public GameObject cornerQuestPanel;      // ★ [신규 추가] 우상단 퀘스트 UI 창 전체를 끄고 켤 부모 오브젝트
    ...
    // 현재 팝업 연출이 돌고 있는지 외부에 알려줄 신호 변수
    ...
    private void Start()
    {
        ...
        // ★ [수정] 게임이 처음 시작될 때는 우상단 UI 창도 완전히 감추어 둡니다!
        if (cornerQuestPanel != null)
            cornerQuestPanel.SetActive(false);
    }
    ...
    public void StartQuest(QuestData quest)
    {
        ...
        // ★ [수정] 첫 번째 퀘스트가 진짜로 시작되는 순간에 우상단 UI 창을 활성화합니다!
        if (cornerQuestPanel != null)
            cornerQuestPanel.SetActive(true);
    ...
    IEnumerator ShowQuestPopUpRoutine(QuestData quest)
    {
        ...
        // 연출이 시작되었으므로 플레이어를 묶기 위해 true로 변경
        isPopUpActive = true; 
        ...
        // 연출이 완벽히 끝났으므로 플레이어를 풀어주기 위해 false로 변경
        isPopUpActive = false; 
    }
    ...
    void UpdateUI()
    {
        ...
        if (statusText != null)
        {
            statusText.text = isCompleted ? "완료" : "진행중"; 
